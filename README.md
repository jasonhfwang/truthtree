...
- Should direct agent to investigate a single topic — e.g. chicken domestication. Then produce a page with a list of notes, facts broken down, and then proposed hierarchy + how they fit in to the global scheme. Basically can be multiple layers of flow graphs. This would be the "editor" or "injector" mode. And then there's view mode, which is a completely different webpage.
